...
from ..mvae.models import Trainer, FeedForwardVAE, ConvolutionalVAE, DiffusionModel
from ..utils import str2bool
from ..mvae.ops.sde import AmbientSphericalGenerative, AmbientSphericalBrownianMotion
...
def main() -> None:
    ...
    print(f"diffusion model: {model_name}; Epochs: {args.epochs}; Time: {cur_time}; Fixed curvature: {args.fixed_curvature}; "
          f"Dataset: {args.dataset}")
    print("#####", flush=True)
    chkpt_dir = f"./chkpt/dfm-{args.dataset}-{model_name}-{cur_time}"
    ...
    model = model_cls(components,args.h_dim,args.epochs,dataset).to(args.device)

    # sde = AmbientSphericalBrownianMotion(args.epochs,components)
    # gsde = AmbientSphericalGenerative(utils.get_drift(model, args.epochs, sde,components))
